...
vs_argmax) and writes:
  - a mean-Sigma table (mean + per-axis between-chain variance at hop 200)
  - the H4-direction contrast (VS-weighted vs VS-argmax on mean Sigma)
...
def _axis_mean(row):
    """Mean across the 5 axes of a sigma row (writes the mean_sigma column)."""
...
def load_sigma(arm):
    ...
    pos["mean_sigma"] = pos[AXES].apply(_axis_mean, axis=1)
    disp["mean_sigma"] = disp[AXES].apply(_axis_mean, axis=1)
    return pos, disp, cos
...
def main():
    for name in ["position", "displacement"]:
        ...
        per = tbl.reset_index().pivot_table(index="config", columns="arm", values="mean_sigma")
            ...
            rows.append(
                {
                    "arm": arm,
                    "metric": name,
                    "mean_sigma": df["mean_sigma"].mean(),
                    "weighted_difference_to_direct": None,
                }
            )
    agg = pd.DataFrame(rows)
    agg.loc[agg["arm"] == "vs_weighted", "weighted_difference_to_direct"] = (
        agg.loc[agg["arm"] == "vs_weighted", "mean_sigma"].values[0]
        - agg.loc[agg["arm"] == "direct", "mean_sigma"].values[0]
    )
    agg.loc[agg["arm"] == "vs_argmax", "weighted_difference_to_direct"] = (
        agg.loc[agg["arm"] == "vs_argmax", "mean_sigma"].values[0]
        - agg.loc[agg["arm"] == "direct", "mean_sigma"].values[0]
    )
    wgt = agg.loc[agg["arm"] == "vs_weighted", "mean_sigma"].values[0]
    amx = agg.loc[agg["arm"] == "vs_argmax", "mean_sigma"].values[0]
    direct = agg.loc[agg["arm"] == "direct", "mean_sigma"].values[0]
